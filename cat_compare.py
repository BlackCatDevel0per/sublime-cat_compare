...
COLOR_SCHEME = 'CatCompareScheme.sublime-color-scheme'
TEMP_THEME = 'CatCompareTheme.tmTheme'

HIDDEN_TEMP_THEME = 'CatCompareTheme.hidden-tempTheme'
HIDDEN_COLOR_SCHEME = 'CatCompareScheme.hidden-color-scheme'

SETTINGS = 'CatCompare.sublime-settings'
...
def plugin_loaded():
	...
	# touch files
	with open(os.path.join(folder, HIDDEN_TEMP_THEME), 'w', encoding='utf-8') as f:
		f.write('')

	with open(os.path.join(folder, HIDDEN_COLOR_SCHEME), 'w', encoding='utf-8') as f:
		f.write('')
...
def delete_old_non_hidden_files():
	...
	# 'Error loading colour scheme Packages/User/CatCompareTheme.tmTheme: Bad XML'

	folder = os.path.join(sublime.packages_path(), 'User')

	filePath = os.path.join(folder, TEMP_THEME)
	if os.path.exists(filePath):
		os.remove(filePath)

	filePath = os.path.join(folder, COLOR_SCHEME)
	if os.path.exists(filePath):
		os.remove(filePath)


# TODO: Mb add option for this..
...
class LayoutPreserver(sublime_plugin.EventListener):
			...
			def after_close():
				...
				if count['nonCompareWin'] is None:
					...
					if int(sublime.version()) < 3000:
						if plugin_settings().get('hide_sidebar', False):
							win.run_command('toggle_side_bar')
						if plugin_settings().get('hide_menu', False):
					...
					sublime.set_timeout(lambda: win.show_quick_panel(
						['Please close all comparison windows first'], None), 10)
...
class MarkSelCommand(sublime_plugin.TextCommand):
	def name(self):
		return 'cat_mark_sel'
	...
	def run(self, edit):
		...
		markedSelection[0] = markedSelection[1]
		markedSelection[1] = selectionText


class CompareFilesCommand(sublime_plugin.ApplicationCommand):
	def name(self):
		return 'cat_compare_files'


	def run(self, A=None, B=None) -> None:
		global files
...
class CompareCommand(sublime_plugin.TextCommand):
	...
	def generate_colour_scheme(self, view, generate=True):
		...
		colour_removed = plugin_settings().get('remove_colour', 'invalid.illegal')
		colour_added = plugin_settings().get('add_colour', 'string')
		colour_modified_deletion = plugin_settings().get(
			'modified_colour_deletion', 'support.class')
		colour_modified_addition = plugin_settings().get(
			'modified_colour_addition', 'support.class')
		colour_unmodified_deletion = plugin_settings().get(
			'unmodified_colour_deletion', 'invalid.illegal')
		colour_unmodified_addition = plugin_settings().get(
			'unmodified_colour_addition', 'string')
		colour_text = plugin_settings().get('text_colour', '')
		...
		if generate:
			# load current scheme
			current_scheme, current_scheme_settings = self.get_current_color_scheme(view)
			try:
				scheme = sublime.load_resource(current_scheme)
				scheme_settings = sublime.load_resource(current_scheme_settings)
			...
			try:
				...
				scheme_settings = sublime.decode_value(scheme_settings)
				scheme_json = True
			except:
				scheme_json = False
			...
			if scheme_json:
				self.apply_scheme_settings(scheme, scheme_settings)
				for name in colourStrings:
					string = colourStrings[name]
					chex = colourHexes[name]
					scheme['rules'].append(
						{"name": name, "scope": string, "background": chex, "foreground": colour_text})
				...
				for name in colourStrings:
					string = colourStrings[name]
					chex = colourHexes[name]
					xml += xml_tmpl.format('Comparison ' +
										   name, string, chex, colour_text)
			...
			theme_name = HIDDEN_TEMP_THEME
			if scheme_json:
				theme_name = HIDDEN_COLOR_SCHEME

			abs_theme_file = os.path.join(
				sublime.packages_path(), 'User', theme_name)
			rel_theme_file = os.path.join(os.path.basename(
				sublime.packages_path()), 'User', theme_name)
	...
	def get_current_color_scheme(self, view) -> 'Tuple[str, str]':
		...
		if not current_scheme.startswith(packages_path + '/'):
			# Mb not work in st under 4..
			rel_current_scheme_files = sublime.find_resources(current_scheme)

			for scheme in rel_current_scheme_files:
				if self.count_dirs(scheme) > 2:
					rel_current_scheme_files.remove(scheme)

			# FIXME: Add exception..
			current_scheme = rel_current_scheme_files[0]
			current_scheme_settings = rel_current_scheme_files[-1]

			current_scheme = current_scheme.replace(
				'\\', '/')
			current_scheme_settings = current_scheme_settings.replace(
				'\\', '/')

		return current_scheme, current_scheme_settings


	# TODO: Mb make static..
	def apply_scheme_settings(self, scheme: dict, scheme_stngs: dict) -> None:
		for key, value in scheme_stngs.items():
			if key not in scheme or not value:
				continue

			if isinstance(value, dict) and key in scheme:
				self.apply_scheme_settings(scheme[key], value)
			elif isinstance(value, list) and key in scheme:
				scheme[key] = value
			else:
				scheme[key] = value
	...
	def get_drawtype(self):
		...
		if int(sublime.version()) >= 3000:
			if not plugin_settings().get('outlines_only', False):
	...
	def highlight_lines(self, view, lines, sublines, col):
		...
		view.add_regions('diff_highlighted-' + col,
						 regionList, colour, '', drawType)
		view.settings().set('markers', markers)
	...
	def sub_highlight_lines(self, view, lines, col):
		...
		drawType = self.get_drawtype()
		view.add_regions('diff_intraline_unmodified-' + col,
						 lineRegionList, colourUnmodified, '', drawType)
		view.add_regions('diff_intraline-' + col,
						 regionList, colour, '', drawType)
	...
	def compare_views(self, view1, view2):
		...
		if plugin_settings().has('ignore_pattern'):
			ignore_pattern = plugin_settings().get('ignore_pattern')
		...
		if plugin_settings().get('ignore_whitespace', False):
			view1_contents = re.sub(r'[ \t]', '', view1_contents)
			view2_contents = re.sub(r'[ \t]', '', view2_contents)

		if plugin_settings().get('ignore_case', False):
		...
		for line in diff:
			...
			if hasIntraline and hasDiffA and hasDiffB:
				if plugin_settings().get('enable_intraline', True):
					...
					for tag, i1, i2, j1, j2 in s.get_opcodes():
						if tag != 'equal':  # == replace
							if plugin_settings().get('intraline_emptyspace', False):
								if tag == 'insert':
									i2 += j2 - j1
								if tag == 'delete':
									j2 += i2 - i1
		...
		if plugin_settings().get('line_count_popup', False):
			numDiffs = len(highlightA) + len(highlightB)
			sublime.message_dialog(intraDiff + str(len(highlightA)) + ' lines removed, ' + str(
				len(highlightB)) + ' lines added\n' + str(numDiffs) + ' line differences total')


	def run(self, edit: 'sublime.Edit', with_active: bool = False, group: int = -1, index: int = -1, compare_selections: bool = False) -> None:
		global markedSelection, files
		...
		def create_comparison(view1_contents: str, view2_contents: str, syntax: str, name1_override: str = '', name2_override: str = '') -> None:
			view1_syntax = syntax
			view2_syntax = syntax
			...
			new_window.set_layout({"cols": [0.0, 0.5, 1.0], "rows": [
			...
			if int(sublime.version()) >= 3000:
				...
				if plugin_settings().get('hide_minimap', False):
					new_window.set_minimap_visible(False)
				if plugin_settings().get('hide_status_bar', False):
					new_window.set_status_bar_visible(False)
				if plugin_settings().get('hide_tabs', False):
			...
			# view names
			view_prefix = plugin_settings().get('display_prefix', '')
			...
			view1.settings().set("is_compare", True)
			view2.settings().set("is_compare", True)
			...
			if plugin_settings().get('read_only', False):
		...
		def on_click(index):
			if index > -1:
				...
				# Strange use..
				create_comparison(view1_contents, view2_contents,
								  syntax, False, openTabs[index][0])

		def compare_from_views(view1, view2):
			if view1.is_loading() or view2.is_loading():
				sublime.set_timeout(
					lambda: compare_from_views(view1, view2), 10)
			else:
				view1_contents = self.get_view_contents(view1)
				view2_contents = self.get_view_contents(view2)
				...
				create_comparison(
					view1_contents, view2_contents, syntax, file1, file2)
		...
		if len(files) > 0:
			...
			del files[:]
		elif compare_selections:
			selA = markedSelection[0]
			selB = markedSelection[1]
			...
			if with_active:
				...
				if group == -1 and index == -1:
					...
					other_active_view = active_window.active_view_in_group(
						group)
...
def scroll_to(view, prev=False):
	if not view.settings().get('is_compare'):
		return

	current_pos = view.sel()[0].begin()
	for col in ['A', 'B']:
		regions = view.settings().get('markers')
		if prev:
			regions.reverse()
...
class PrevDiffCommand(sublime_plugin.TextCommand):
	def run(self, edit, string='') -> None:
		scroll_to(self.view, prev=True)

	def name(self):
		return 'cat_prev_diff'


class NextDiffCommand(sublime_plugin.TextCommand):
	def run(self, edit, string='') -> None:
		scroll_to(self.view)

	def name(self):
		return 'cat_next_diff'


class SelectTextCommand(sublime_plugin.TextCommand):
	def run(self, edit, index='') -> None:
		window = self.view.window()

		if index == '':
			menu_items = ['Select removed text', 'Select added text']
			window.show_quick_panel(menu_items, lambda i: window.run_command(
				'select_text', {'index': i}))
			return

		view = window.views()[index]
		view.sel().clear()
		regions = view.get_regions('diff_highlighted-A') + view.get_regions('diff_highlighted-B') + \
